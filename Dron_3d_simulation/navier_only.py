#!/usr/bin/env python3
"""
PINN without Zero Equation turbulence (drone + propellers inside cube)
"""
import os
import re
import csv
...
import matplotlib
matplotlib.use('Agg')  # If no GUI is needed
...
def run(cfg: ModulusConfig):
    ...
    # for lamina fluid
    ns_eq = NavierStokes(nu=nu_phys, rho=rho_phys, dim=3, time=False)
    ...
    # for laminar fluid
    nodes = (ns_eq.make_nodes()+[flow_net.make_node(name="flow_network")])
    ...
    compute_mse_stats(
        vtp_path=os.path.join(os.path.dirname(__file__),
                              "outputs","navier_only","validators","foam_validator.vtp")
    )
